...
namespace ArkSharp
{
	...
	public static class HashHelper
	{
		...
		public static string MD5(byte[] bytes)
		{
			...
			{
				if (_emptyMD5 == null)
					_emptyMD5 = MD5(_emptyBuffer, 0, 0);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static string MD5(byte[] bytes, int offset, int count)
		{
			if (bytes == null)
				bytes = _emptyBuffer;
			...
			var output = md5.ComputeHash(bytes, offset, count);
		...
		public static int GetHash32(string s)
		{
			unchecked
			{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static ushort GetHash16(string s)
		{
			return (ushort)(GetHash32(s) & 0xffff);
		}
		...
		public static uint CRC32(byte[] bytes, int offset, int count)
		{
			...
			for (int i = offset; i < offset + count; ++i)
				crc = (crc >> 8) ^ _crc32Table[bytes[i] ^ crc & 0xff];
		...
		public static uint CRC32(Stream stream)
		{
			...
			for (;;)
			{
				...
				for (int i = 0; i < readCount; ++i)
					crc = (crc >> 8) ^ _crc32Table[buffer[i] ^ crc & 0xff];
			}
